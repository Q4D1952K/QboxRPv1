if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
    ...
    RegisterNetEvent('cdn-fuel:station:server:Withdraw', function(amount, location, StationBalance)
        ...
        if Config.FuelDebug then print("Attempting to withdraw $" .. amount ..
            " from Location #" .. location .. "'s Balance!") end
        if amount > StationBalance then
            exports.qbx_core:Notify(src, Lang:t("station_withdraw_too_much"), 'success')
            return
        end
    ...
    RegisterNetEvent('cdn-fuel:station:server:Deposit', function(amount, location, StationBalance)
        ...
        local Player = QBX:GetPlayer(src)
        local setamount = (StationBalance + amount)
        if Config.FuelDebug then print("Attempting to deposit $" .. amount .. " to Location #" .. location ..
            "'s Balance!") end
    ...
    RegisterNetEvent('cdn-fuel:station:server:updatefuelprice', function(fuelprice, location)
        local src = source
        if Config.FuelDebug then print('Attempting to update Location #' ..
            location .. "'s Fuel Price to a new price: $" .. fuelprice) end
    ...
    RegisterNetEvent('cdn-fuel:station:server:updatereserves', function(reason, amount, currentlevel, location)
        ...
        if Config.FuelDebug then print('Attempting to ' ..
            reason .. ' ' .. amount .. ' to / from Location #' .. location .. "'s Reserves!") end
    ...
    RegisterNetEvent('cdn-fuel:station:server:updatebalance',
        function(reason, amount, StationBalance, location, FuelPrice)
            ...
            if Config.FuelDebug then print('Attempting to ' ..
                reason .. ' ' .. StationGetAmount .. ' to / from Location #' .. location .. "'s Balance!") end
    ...
    RegisterNetEvent('cdn-fuel:stations:server:buyreserves', function(location, price, amount)
            ...
            for k, v in pairs(result) do
                ...
                if v.fuel + amount > Config.MaxFuelReserves then
                    ...
                    if Config.FuelDebug then print(
                        "Purchase is not possible, as reserves will be greater than the maximum amount!") end
                ...
                elseif v.fuel + amount <= Config.MaxFuelReserves then
                    ...
                    if Config.FuelDebug then print(
                        "Purchase is possible, as reserves will be below or equal to the maximum amount!") end
                else
        ...
        if Config.FuelDebug then print("Attempting Sale Server Side for location: #" .. location ..
            " for Price: $" .. price) end
        if ReserveBuyPossible and Player.Functions.RemoveMoney("bank", price, "Purchased" .. amount .. "L of Reserves for: " .. Config.GasStations[location].label .. " @ $" .. Config.FuelReservesPrice .. " / L!") then
            if not Config.OwnersPickupFuel then
                ...
                if Config.FuelDebug then print("SQL Execute Update: fuel_station level to: " ..
                    NewAmount .. " Math: (" .. amount .. " + " .. OldAmount .. " = " .. NewAmount) end
            else
                ...
                if Config.FuelDebug then print("Initiating a Fuel Pickup for Location: " ..
                    location .. " with for the amount of " .. NewAmount .. " | Triggered By: Source: " .. src) end
            end
    ...
    RegisterNetEvent('cdn-fuel:station:server:fuelpickup:failed', function(location)
        ...
        if location then
            if FuelPickupSent[location] then
                local cid = QBX:GetPlayer(src).PlayerData.citizenid
        ...
        if result then
            for k, v in pairs(result) do
                ...
                elseif v.owned == 0 then
                    ...
                    if Config.FuelDebug then print(
                        "Owned State (v.owned ~= 1 or 0) It must be 1 or 0! 1 = True, 0 = False!") end
                end
                cb(owned)
            end
        else
