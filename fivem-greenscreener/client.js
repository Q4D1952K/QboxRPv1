...
async function takeScreenshotForComponent(pedType, type, component, drawable, texture, cameraSettings, _type) {
...
function stopWeatherResource() {
	...
	if ((GetResourceState('qb-weathersync') == 'started') || (GetResourceState('qbx_weathersync') == 'started') || (GetResourceState('Renewed-Weathersync') == 'started')) {
		...
		return true;
	} else if (GetResourceState('vSync') == 'started' ) {
						...
						for (let drawable = 0; drawable < drawableVariationCount; drawable++) {
							...
							if (config.includeTextures) {
								for (let texture = 0; texture < textureVariationCount; texture++) {
									...
									await takeScreenshotForComponent(pedType, type, component, drawable, texture, false, config.cameraSettings[type][component].type);
								}
							} else {
						...
						for (let prop = 0; prop < propVariationCount; prop++) {
							...
							if (config.includeTextures) {
								for (let texture = 0; texture < textureVariationCount; texture++) {
									...
									await takeScreenshotForComponent(pedType, type, component, prop, texture,false, config.cameraSettings[type][component].type);
								}
							} else {
